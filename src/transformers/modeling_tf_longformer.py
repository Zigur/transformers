...
from .modeling_tf_outputs import TFBaseModelOutputWithPooling, TFMaskedLMOutput, TFQuestionAnsweringModelOutput, \
    TFSequenceClassifierOutput, TFTokenClassifierOutput, TFMultipleChoiceModelOutput
from .modeling_tf_roberta import TFRobertaEmbeddings, TFRobertaLMHead, TFRobertaClassificationHead
...
""""
This is commented out as I am just using TFRobertaClassificationHead for now
...
class TFLongformerForSequenceClassification(TFLongformerPreTrainedModel, TFSequenceClassificationLoss):
    ...
    def call(
        ...
        if return_dict is None:
            return_dict = self.longformer.return_dict
...
class TFLongformerForTokenClassification(TFLongformerPreTrainedModel, TFTokenClassificationLoss):
    ...
    def call(
        ...
        # FIXME is something like this needed here?
        """
        if global_attention_mask is None:
            logger.info("Initializing global attention on CLS token...")
            global_attention_mask = tf.zeros_like(inputs)
...
class TFLongformerForMultipleChoice(TFLongformerPreTrainedModel, TFMultipleChoiceLoss):
    ...
    def dummy_inputs(self):
        ...
        """
        return dict(input_ids=tf.constant(MULTIPLE_CHOICE_DUMMY_INPUTS))
    ...
    def call(
        ...
        flat_global_attention_mask = tf.reshape(
            global_attention_mask, (-1, seq_length)
